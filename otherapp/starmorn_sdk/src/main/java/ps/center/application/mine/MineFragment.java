...
import ps.center.databinding.BusinessFragmentMineBinding;
...
/* loaded from: classes.jar:ps/center/application/mine/MineFragment.class */
public class MineFragment extends BaseFragmentVB<BusinessFragmentMineBinding> implements DataChangeCallBack, FreeManager.FreeTimeOverListener {
    ...
    public BusinessFragmentMineBinding getLayout() {
        return BusinessFragmentMineBinding.inflate(getLayoutInflater());
    }
    ...
    protected void initData(BundleGet bundleGet) {
        ...
        this.settingListAdapter = new SettingListAdapter(getContext(), R.layout.business_item_mine_settings_layout, this.settingConfig, this.listData);
        ((BusinessFragmentMineBinding) this.binding).settings.setLayoutManager(new LinearLayoutManager(getContext()));
        ((BusinessFragmentMineBinding) this.binding).settings.setAdapter(this.settingListAdapter);
        ...
        if (pay_page.comm.is_active.equals("1")) {
            if (pay_page.func.backpay_sw.equals("1")) {
                ((BusinessFragmentMineBinding) this.binding).vipBannerLayout.setVisibility(View.VISIBLE);
            } else {
                ((BusinessFragmentMineBinding) this.binding).vipBannerLayout.setVisibility(View.GONE);
            }
        } else {
            ((BusinessFragmentMineBinding) this.binding).vipBannerLayout.setVisibility(View.GONE);
        }
        ((BusinessFragmentMineBinding) this.binding).rootLayout.setBackgroundColor(Color.parseColor(this.settingConfig.minePageThemeColor));
        ((BusinessFragmentMineBinding) this.binding).topLayout.setImageResource(this.settingConfig.mineTopThemeImage);
        ((BusinessFragmentMineBinding) this.binding).vipBannerLayout.setBackgroundResource(this.settingConfig.mineVipImage);
        ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setTextColor(Color.parseColor(this.settingConfig.mineVipBtnTextColor));
        ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setBackgroundResource(this.settingConfig.mineVipBtnDrawable);
        ((BusinessFragmentMineBinding) this.binding).vipBannerTitle.setTextColor(Color.parseColor(this.settingConfig.mineVipTitleTextColor));
        ((BusinessFragmentMineBinding) this.binding).vipBannerDesc.setTextColor(Color.parseColor(this.settingConfig.mineVipTipsTextColor));
        ((BusinessFragmentMineBinding) this.binding).userName.setTextColor(Color.parseColor(this.settingConfig.mineNicknameColor));
        ((BusinessFragmentMineBinding) this.binding).userInfoGotoImg.setImageResource(this.settingConfig.mineUserInfoJumpBtnDrawable);
        ...
        if (login_mode.comm.is_active.equals("1")) {
            ((BusinessFragmentMineBinding) this.binding).userInfoLayout.setVisibility(View.VISIBLE);
        } else {
            ((BusinessFragmentMineBinding) this.binding).userInfoLayout.setVisibility(View.GONE);
        }
        if (this.settingConfig.mineVipBannerLogoIcon == 0) {
            ((BusinessFragmentMineBinding) this.binding).vipBannerLogo.setVisibility(View.GONE);
            ((BusinessFragmentMineBinding) this.binding).paddingView.setVisibility(View.VISIBLE);
        } else {
            ((BusinessFragmentMineBinding) this.binding).vipBannerLogo.setVisibility(View.VISIBLE);
            ((BusinessFragmentMineBinding) this.binding).paddingView.setVisibility(View.GONE);
            ((BusinessFragmentMineBinding) this.binding).vipBannerLogo.setImageResource(this.settingConfig.mineVipBannerLogoIcon);
        }
    ...
    protected void setListener() {
        ((BusinessFragmentMineBinding) this.binding).userInfoLayout.setOnClickListener(v -> {
            if (getActivity() != null) {
                LoginManager.login(getActivity(), 0);
            }
        });
        ((BusinessFragmentMineBinding) this.binding).vipBannerLayout.setOnClickListener(v2 -> {
        ...
        this.settingListAdapter.setOnItemClickListener((position, item) -> {
            switch (item.name) {
                ...
                case "关于我们":
                    ...
                    break;
                case "邀请好友":
                    BusHttp.share().getShareInfo("1", BusinessConstant.getConfig().standard_conf.share_control.func.share_id, new Result<ShareBean>() { // from class: ps.center.application.mine.MineFragment.1
                ...
                case "免费试玩":
                    FreeManager.get().receiveFreeUser(new FreeManager.ReceiveFreeVipListener() { // from class: ps.center.application.mine.MineFragment.2
    ...
    private void setUserData(User userData) {
        this.listData.clear();
        Glide.with(Super.getContext()).load(userData.avatar).placeholder(this.settingConfig.mineUserDefaultIcon).error(this.settingConfig.mineUserDefaultIcon).apply(new RequestOptions().centerCrop().circleCrop()).into(((BusinessFragmentMineBinding) this.binding).userIcon);
        if (CenterConstant.getUser().isSign) {
            ((BusinessFragmentMineBinding) this.binding).userInfoGotoImg.setVisibility(View.VISIBLE);
        } else {
            ((BusinessFragmentMineBinding) this.binding).userInfoGotoImg.setVisibility(View.GONE);
        }
        ((BusinessFragmentMineBinding) this.binding).userName.setText(userData.username);
        if (userData.isVip) {
            ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setText("续费");
            if (userData.vip_time == 4092595200L) {
                ((BusinessFragmentMineBinding) this.binding).vipBannerTitle.setText("您已是永久超级会员 ");
                ((BusinessFragmentMineBinding) this.binding).vipBannerDesc.setText("到期时间:终身使用");
                ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setVisibility(View.GONE);
            } else {
                ((BusinessFragmentMineBinding) this.binding).vipBannerTitle.setText("您已是超级会员 ");
                ((BusinessFragmentMineBinding) this.binding).vipBannerDesc.setText(String.format("到期时间:%s", TimeUtils.timeToData(userData.vip_time * 1000).split(" ")[0]));
                ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setVisibility(View.VISIBLE);
            }
        } else {
            ((BusinessFragmentMineBinding) this.binding).vipBannerBtn.setText("开通");
            ((BusinessFragmentMineBinding) this.binding).vipBannerTitle.setText("成为超级会员 ");
            ((BusinessFragmentMineBinding) this.binding).vipBannerDesc.setText("享受所有会员权益");
        }
    ...
    private void getIdentityAuthentication() {
        ...
        BusHttp.common().identityAuthentication(id_card_name, id_card_number, new Result<IdentityAuthentication>() { // from class: ps.center.application.mine.MineFragment.3
            @Override // ps.center.library.http.base.Result
            public void success(IdentityAuthentication obj) {
                super.success(obj);
                MineFragment.this.settingListAdapter.setIdentityAuthentication(obj.isAuthentication());
                MineFragment.this.settingListAdapter.notifyDataSetChanged();
    ...
    public void timeOver() {
        CenterHttp.get().getUserInfo(new Result<User>() { // from class: ps.center.application.mine.MineFragment.4
            @Override // ps.center.library.http.base.Result
